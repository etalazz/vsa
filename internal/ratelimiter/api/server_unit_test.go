...
func TestServer_ReleaseEndpoint_RefundFlow(t *testing.T) {
    ...
    for i := 0; i < int(rateLimit); i++ {
        ...
        if err != nil { t.Fatalf("/check consume %d: %v", i+1, err) }
    ...
    resp, err := client.Get(ts.URL + "/check?api_key=" + key)
    if err != nil { t.Fatalf("/check after limit: %v", err) }
    ...
    if err != nil { t.Fatalf("/release refund: %v", err) }
    ...
    resp, err = client.Get(ts.URL + "/check?api_key=" + key)
    if err != nil { t.Fatalf("/check after refund: %v", err) }
    if resp.StatusCode != http.StatusOK {
    ...
    resp, err = client.Get(ts.URL + "/check?api_key=" + key)
    if err != nil { t.Fatalf("/check final: %v", err) }
    if resp.StatusCode != http.StatusTooManyRequests {
...
func TestServer_ReleaseEndpoint_MissingKey(t *testing.T) {
    ...
    resp, err := client.Do(req)
    if err != nil { t.Fatalf("/release without key: %v", err) }
